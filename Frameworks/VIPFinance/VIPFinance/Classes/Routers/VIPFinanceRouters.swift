...
public class VIPFinanceRouters: IAppProductRouter {
    let router: IAppRouter

    let modules: [String: (IAppRouter) -> IAppModule] = [
        VIPFinanceProducts.HomeModule.routePath: { HomeModule($0) },
        VIPFinanceProducts.DetailModule.routePath: { DetailModule($0) },
        VIPFinanceProducts.TransactionModule.routePath: { TransactionModule($0) }
    ]
